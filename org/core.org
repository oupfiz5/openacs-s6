...
- [[#quickstart][Quickstart]]
  - [[#manage-openacs-using-docker-composeyaml][Manage OpenACS using docker-compose.yaml]]
...
:PROPERTIES:
:CUSTOM_ID: manage-openacs-using-docker-compose-yaml
:END:
We use [[https://hub.docker.com/_/postgres][official posgres]] image in docker-compose.yaml file.
*** Prerequisite
:PROPERTIES:
:CUSTOM_ID: prerequisite
:END:
...
:END:
start:
#+begin_src sh :dir ../src/ :tangle no
docker-compose up
#+end_src
...
|-----------------------+--------------------+-------------------------------------|
| [[*OpenACS listen port][OACS_LISTEN_PORT]]      | 8080               | Set http listen port, example 8080  |
|-----------------------+--------------------+-------------------------------------|
...
#+begin_src sh :shebang ""
  # * From
  ARG NS_IMAGE_TAG=${NS_IMAGE_TAG:-4.99.23}
  FROM oupfiz5/naviserver-s6:${NS_IMAGE_TAG}
...
#+begin_src yaml
  ...
  services:
    openacs:
      image: "oupfiz5/openacs-s6:${OACS_IMG_TAG}"
      env_file:
        - .env
      volumes:
...
#+begin_src sh :shebang "" :tangle-mode (identity #o644)
  ...
  OACS_IMG_TAG="${OACS_IMG_TAG:-oacs-5-10-1}"
...
** .env.datacore :code:
:PROPERTIES:
:CUSTOM_ID: -env-datacore
:END:
#+begin_src sh :shebang "" :tangle-mode (identity #o644)
  POSTGRES_REPOSITORY="datacore/postgresql"
  POSTGRES_TAG="14.1-alpine"
#+end_src

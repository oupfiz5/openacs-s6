...
#+begin_src sh :shebang ""
  CONTAINER_NAME="${CONTAINER_NAME:-openacs-s6}"
  NS_IMAGE_REPOSITORY="${NS_IMAGE_REPOSITORY:-oupfiz5}"
  NS_IMAGE_NAME="${NS_IMAGE_NAME:-naviserver-s6}"
  NS_IMAGE_TAG="${NS_IMAGE_TAG:-20.04-3.0.0.2-4.99.23}"
  IMAGE_NAME="${IMAGE_NAME:-openacs-s6}"
  IMAGE_REPOSITORY="${IMAGE_REPOSITORY:-oupfiz5}"
  IMAGE_TAG="${IMAGE_TAG:-s6v3-oacs-5-10-1}"
  ...
  jobs:
    build:
      ...
      steps:
        ...
        - name: s6v2. OpenACS. Build and push image.
          run: |
            echo "::group::Prepare environment"
            set -a; source ${GITHUB_WORKSPACE}/src/VERSIONS ; set +a;
            export IMAGE_TAG="s6v2-oacs-5-10-1"
            cd ${GITHUB_WORKSPACE}/src/hook
            echo "::endgroup::"
            echo "::group::Build image"
            ./build.sh
    ...
    jobs:
      checks:
        ...
        steps:
          ...
          - name: s6v2. OpenACS. Build. Check image and container.
            run: |
              echo "::group::Build image"
              set -a; source ${GITHUB_WORKSPACE}/src/VERSIONS ; set +a;
              export IMAGE_TAG="s6v2-oacs-5-10-1"
...
#+begin_src sh :shebang "" :tangle-mode (identity #o644)
  ...
  OACS_IMAGE_TAG="${OACS_IMAGE_TAG:-s6v3-oacs-5-10-1}"

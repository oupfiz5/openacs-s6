#+title: OpenACS on ubuntu using S6.
...
:CUSTOM_ID: about
:END:
This is [[https://openacs.org/][OpenACS]] on [[https://hub.docker.com/_/ubuntu][Ubuntu base docker image]] (version 20.04) using [[https://github.com/just-containers/s6-overlay][s6-overlay]].  To install OpenACS I used  code from  [[https://github.com/gustafn/install-ns][Gustaf Neumann]] script [[https://github.com/gustafn/install-ns/blob/master/install-oacs.sh][install-oacs.sh]]. The base image is [[https://hub.docker.com/r/oupfiz5/openacs-s6][oupfiz5/openacs-s6]].
...
- [[#quickstart][Quickstart]]
  - [[#manage-openacs-using-docker-compose-dbyaml][Manage OpenACS using docker-compose-db.yaml]]
    ...
    - [[#prune-all][Prune all]]
  - [[#deprecate-manage-openacs-using-docker-compose-datacoreyaml][Deprecate. Manage OpenACS using docker-compose-datacore.yaml]]
    - [[#prerequisite-2][Prerequisite]]
    - [[#startstop-interactive-1][Start/stop interactive]]
    - [[#startstopremove-as-daemon-1][Start/stop/remove as daemon]]
    - [[#view-logs-1][View logs]]
    - [[#view-list-containers-1][View list containers]]
    - [[#prune-all-1][Prune all]]
  - [[#manage-openacs-using-docker-compose-official-dbyaml][Manage OpenACS using docker-compose-official-db.yaml]]
    - [[#prerequisite-3][Prerequisite]]
    - [[#startstop-interactive-2][Start/stop interactive]]
    - [[#startstopremove-as-daemon-2][Start/stop/remove as daemon]]
    - [[#view-logs-2][View logs]]
    - [[#view-list-containers-2][View list containers]]
    - [[#prune-all-2][Prune all]]
...
- [[#code][Code]]
  ...
  - [[#fossil-settingscrlf-glob-1][../.fossil-settings/crlf-glob]]
  ...
  - [[#rootfsetccont-initd01-wait-postgres-onetime][rootfs/etc/cont-init.d/01-wait-postgres-onetime]]
  - [[#rootfsetcservicesdnaviserverrun][rootfs/etc/services.d/naviserver/run]]
  - [[#rootfsusrlocalnsconfopenacs_configtcl][rootfs/usr/local/ns/conf/openacs_config.tcl]]
  - [[#rootfsusrlocalnsconfconfig_varstcl][rootfs/usr/local/ns/conf/config_vars.tcl]]
  - [[#rootfsvarwwwopenacswwwsystemopenacs-testtcl][rootfs/var/www/openacs/www/SYSTEM/openacs-test.tcl]]
  - [[#rootfsusrlocalnsconfinstall-oacs-core-configxml][rootfs/usr/local/ns/conf/install-oacs-core-config.xml]]
  - [[#hookbuildsh][hook/build.sh]]
  - [[#hookpushsh][hook/push.sh]]
  - [[#docker-compose-datacoreyaml][docker-compose-datacore.yaml]]
  - [[#docker-compose-official-dbyaml][docker-compose-official-db.yaml]]
...
:END:
 ...
 |------------+-----------+-----------------------------|
 | OACS_TAG   | oacs-5-10 | Set default OpenACS version |
 |------------+-----------+-----------------------------|
...
:PROPERTIES:
:CUSTOM_ID: manage-openacs-using-docker-compose-db-yaml
:END:
We use [[https://hub.docker.com/_/postgres][official posgres]] image in docker-compose-official-db.yaml file.
*** Prerequisite
:PROPERTIES:
:CUSTOM_ID: prerequisite
:END:
...
:PROPERTIES:
:CUSTOM_ID: deprecate--manage-openacs-using-docker-compose-datacore-yaml
:END:
Pay attention: now it is deprecate
We use postgres image from [[https://hub.docker.com/r/datacore/postgresql][datacore/postgresql]] in docker-compose.yaml file.
*** Prerequisite
:PROPERTIES:
:CUSTOM_ID: prerequisite
:END:
...
:PROPERTIES:
:CUSTOM_ID: manage-openacs-using-docker-compose-official-db-yaml
:END:
We use [[https://hub.docker.com/_/postgres][official posgres]] image in docker-compose-official-db.yaml file.
*** Prerequisite
:PROPERTIES:
...
|-----------------------+--------------------+-------------------------------------|
| [[*Timezone][TZ]]                    | UTC                | Set timezone, example Europe/Moscow |
|-----------------------+--------------------+-------------------------------------|
| [[*OpenACS listen port][OA_LISTEN_PORT]]        | 8080               | Set http listen port, example 8080  |
|-----------------------+--------------------+-------------------------------------|
| [[*NaviServer configuration file][NS_CONF]]               | openacs_config.tcl | Configuration file for NaviServer   |
|-----------------------+--------------------+-------------------------------------|
| [[*NaviServer variable file][NS_VARS]]               | config_vars.tcl    | Variable file for NaviServer        |
|-----------------------+--------------------+-------------------------------------|
| [[*Database hostname][DB_HOST]]               | postgres           | Database host name                  |
|-----------------------+--------------------+-------------------------------------|
| [[*Database username][DB_USER]]               | openacs            | Database user name                  |
|-----------------------+--------------------+-------------------------------------|
| [[*Database password][DB_PASS]]               | testing            | Database user password              |
|-----------------------+--------------------+-------------------------------------|
| [[*Postgres official tag][POSTGRES_OFFICIAL_TAG]] | 10-alpine          | Docker tag from official postgres.  |
|-----------------------+--------------------+-------------------------------------|
| [[*Postgres datacore tag][POSTGRES_DATACORE_TAG]] | 10.12              | Docker tag from datacore/postgresql |
|-----------------------+--------------------+-------------------------------------|
...
:PROPERTIES:
:CUSTOM_ID: naviserver-variable-file
:END:
...
:PROPERTIES:
:CUSTOM_ID: postgres-official-tag
:END:
=POSTGRES_OFFICIAL_TAG= set the docker image tag for [[https://hub.docker.com/_/postgres][official postgres]]. Using only for =docker-compose-official-db.yaml=  Pay attention - some openacs version can use only correspond version of postgres.
...
:PROPERTIES:
:CUSTOM_ID: postgres-datacore-tag
:END:
=POSTGRES_DATACORE_TAG= set the docker image tag for [[https://hub.docker.com/r/datacore/postgresql][datacore postgres]]. Using only for =docker-compose.yaml=. Pay attention - some openacs version can use only correspond version of postgres.
...
#+end_src
** rootfs/usr/local/ns/conf/openacs_config.tcl :code:
:PROPERTIES:
...
#+end_src
** rootfs/usr/local/ns/conf/config_vars.tcl :code:
:PROPERTIES:
:CUSTOM_ID: rootfs-usr-local-ns-conf-config-vars-tcl
:END:
...
#+end_src
** rootfs/usr/local/ns/conf/nsd-config.tcl :noexport:code:notangle:
:PROPERTIES:
:CUSTOM_ID: rootfs-usr-local-ns-conf-nsd-config-tcl
:END:

#+begin_src tcl :shebang "" :tangle-mode (identity #o644) :tangle no
  ########################################################################
...
** docker-compose-datacore.yaml :code:
:PROPERTIES:
:CUSTOM_ID: docker-compose-datacore-yaml
:END:
#+begin_src yaml
  version: '3.9'
  ...
  services:
    openacs:
      image: oupfiz5/openacs-s6:${OACS_TAG}
      env_file:
        - .env
      volumes:
...
:PROPERTIES:
:CUSTOM_ID: docker-compose-official-db-yaml
:END:
#+begin_src yaml
  version: '3.9'
...
#+begin_src sh :shebang "" :tangle-mode (identity #o644)
  ...
  POSTGRES_DB="${DB_NAME}"
  POSTGRES_OFFICIAL_TAG="14.1-alpine"
